...
#define PROFILES_LENGTH 3 //max number of profiles, limited by memory
...
profile EEMEM profilesEE[PROFILES_LENGTH] = {
    ...
    //{"ACV #1         ",  ACV, 100, 50, 50, 0, -550, 4, RANGE_10UA},
    //{"CA #1          ",  CA,  1000, 500, 250, 1000, 500, 1, RANGE_10UA},
    //{"ConstantVoltage",  CONSTVOLT, -550, 60, 0, 0, 0, 0, RANGE_10UA},
    //{"SWV #2         ",  SWV, 60, 0,-500, 50, 3, 0, RANGE_10UA},
    //{"CV #2          ",  CV, 10, 100, -100, 1, 1, 0, RANGE_10UA},
    //{"LSV #2         ",  LSV, 60, 0, -500, 50, 3, 0, RANGE_10UA},
    //{"ACV #2         ",  ACV, 100, 50, 100, 0, -500, 100, RANGE_10UA},
    //{"CA #2          ",  CA, 1000, -500, 250, 1000, 500, 1, RANGE_10UA}
};
...
void printInt16(int16_t n, uint8_t base);
void printInt16(int16_t n, uint8_t base);
    ...
    while(1)
    {
        ...
        else if(status == PROFILE_TEST)
        {
            ...
            if(profiles[profile_index].type == SWV)
                ...
                length = CONSTVOLT_long(profiles[profile_index].name, profiles[profile_index].op1, profiles[profile_index].op2, profiles[profile_index].curr_range);
...
int buttonHandler(profile profiles[PROFILES_LENGTH], uint8_t* status, uint8_t* profile_index, uint8_t* profile_opt_index, uint8_t* profile_edit_index, uint8_t* profile_edit_sel, int16_t* length)
{
    ...
    else if(*status == PROFILE_EDIT)
    {
        ...
        else if(dir == DOWN)
        {
            ...
            {
                if (
                        (*profile_edit_index < 6 && profiles[*profile_index].type != CONSTVOLT)      || 
                        ...
                        (*profile_edit_index < 2 && profiles[*profile_index].type == CONSTVOLTLONG)  
                        )
                    (*profile_edit_index)++;	
            }
            else if(*profile_edit_index == 0)
